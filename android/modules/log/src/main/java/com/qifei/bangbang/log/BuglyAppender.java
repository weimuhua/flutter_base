...
public class BuglyAppender extends UnsynchronizedAppenderBase<ILoggingEvent> {
    /**
     * Max tag length enforced by Android http://developer.android.com/reference/android/util/Log
     * .html#isLoggable(java.lang.String, int)
     */
        ...
        switch (event.getLevel().levelInt) {
            ...
            case Level.TRACE_INT:
                if (!checkLoggable || Log.isLoggable(tag, Log.VERBOSE)) {
                    BuglyLog.v(tag, this.encoder.getLayout().doLayout(event));
                }
                break;

            case Level.DEBUG_INT:
                if (!checkLoggable || Log.isLoggable(tag, Log.DEBUG)) {
                    BuglyLog.d(tag, this.encoder.getLayout().doLayout(event));
                }
                break;

            case Level.INFO_INT:
                if (!checkLoggable || Log.isLoggable(tag, Log.INFO)) {
                    BuglyLog.i(tag, this.encoder.getLayout().doLayout(event));
                }
                break;

            case Level.WARN_INT:
                if (!checkLoggable || Log.isLoggable(tag, Log.WARN)) {
                    BuglyLog.w(tag, this.encoder.getLayout().doLayout(event));
                }
                break;

            case Level.ERROR_INT:
                if (!checkLoggable || Log.isLoggable(tag, Log.ERROR)) {
                    BuglyLog.e(tag, this.encoder.getLayout().doLayout(event));
                }
                break;
     ...
     * <p>
     * The expanded text of the pattern must be less than 23 characters as limited by Android.
     * Layouts that exceed this limit are truncated, and a star is appended to the tag to indicate
     * this.
     * <p>
     * The <code>tagEncoder</code> result is limited to 22 characters plus a star to indicate
     * truncation (<code>%logger{0}</code> has no length limit in logback, but
     * <code>LogcatAppender</code> limits the length internally). For example, if the
     * <code>tagEncoder</code> evaluated to <code>foo.foo.foo.foo.foo.bar.Test</code>, the tag seen
     * in Logcat would be: <code>foo.foo.foo.foo.foo.ba*</code>. Note that <code>%logger{23}</code>
     * yields more useful results (in this case: <code>f.f.foo.foo.bar.Test</code>).
     *
     * @param encoder the pattern-layout encoder; specify {@code null} to automatically use the
     *                logger's name as the tag
     */
     ...
     * <p>
     * See http://developer.android.com/reference/android/util/Log.html#isLoggable(java.lang.String,
     * int)
     *
    ...
    protected String getTag(ILoggingEvent event) {
        ...
        String tag = (this.tagEncoder != null) ? this.tagEncoder.getLayout().doLayout(event)
                : event.getLoggerName();
